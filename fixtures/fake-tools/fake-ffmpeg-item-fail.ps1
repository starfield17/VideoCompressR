...
if ($joined -match '-encoders') { Write-Output ' V..... libx265 fake encoder'; Write-Output ' V..... hevc_qsv fake encoder'; exit 0 }
if ($joined -match '-hwaccels') { Write-Output 'Hardware acceleration methods:'; exit 0 }
if ($joined -match '-h encoder=') { Write-Output '       slow'; Write-Output '       medium'; exit 0 }
if (($joined -match 'fail-item') -and ($joined -match 'pipe:1')) { [Console]::Error.WriteLine('item failure'); exit 17 }
...
if ($joined -match 'missing-output-item') { exit 0 }
